...
import { computed } from 'vue';
    ...
    const props = defineProps( {
        isDashboardConfigActive : {type: Boolean, default: false}
    } )

    const emit = defineEmits( [ 'dashboardConfigTrigger' ] )

    const dashboardConfigClasses = computed(() => props.isDashboardConfigActive ? 'right-0' : '-right-90')
...
<template>
 <div >
      <a @click="emit('dashboardConfigTrigger')"
      ...
      <div
       :class="dashboardConfigClasses"
       class="z-sticky backdrop-blur-2xl backdrop-saturate-200 dark:bg-slate-850/80 shadow-3xl w-90 ease  fixed top-0 left-auto flex h-full min-w-0 flex-col break-words rounded-none border-0 bg-white/80 bg-clip-border px-2.5 duration-200"
       >
