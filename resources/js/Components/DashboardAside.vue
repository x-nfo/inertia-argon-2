<script setup>
    ...
    const props = defineProps({
        ...
        isDarkSidenavActive: { type: Boolean, default: false },
    } );
    ...
    const mobileSidenavClasses = computed(() =>
        props.isMobileSidenavActive
            ? 'xl:ml-6  translate-x-0 ml-6'
            : 'shadow-xl  xl:ml-6'
    );

    const darkSidenavClasses = computed( () =>
        props.isDarkSidenavActive
            ? 'bg-slate-850 shadow-none dark'
            : 'bg-white shadow-xl'
    );

    //Background Sidebar Active Color
    const { selectedColor } = useSidebarColor();
...
<template>
    <!-- sidenav  -->
    <aside class="fixed inset-y-0 flex-wrap items-center justify-between block w-full p-0 my-4 overflow-y-auto antialiased transition-transform duration-200 -translate-x-full border-0 dark:shadow-none dark:bg-slate-850 xl:ml-6 max-w-64 ease-nav-brand z-990 rounded-2xl xl:left-0 xl:translate-x-0 ps"
        :class="[mobileSidenavClasses, darkSidenavClasses]"
        :aria-expanded="props.isMobileSidenavActive"
    >
        <div class="h-19">
            <i
            ...
            <ul class="flex flex-col pl-0 mb-0">
                <li class="mt-0.5 w-full">
                    ...
                    >
                        ...
                        >
                            <i
                                class="relative top-0 text-sm leading-normal text-blue-500 dark:text-blue-300 ni ni-tv-2"
                            ></i>
                        </div>
                        <span
                ...
                <li class="mt-0.5 w-full">
                    ...
                    >
                        ...
                        >
                            <i
                                class="relative top-0 text-sm leading-normal text-slate-700 dark:text-slate-200 ni ni-single-02"
                            ></i>
                        </div>
                        <span
                            class="ml-1 duration-300 opacity-100 pointer-events-none ease"
